...
#     st.write (algorithm_name+ " algorithm results")
    ...
    return_result['id']=run_id
    return_result['algorithm']=algorithm_name
    return_result['dataset']="artis10000"
    return_result['roc_ys']=[1.2,1.2,1.2,1.2,1.2] # Mock 
    return_result['actual_0']=conf_matrix_results[0]
    return_result['actual_1']=conf_matrix_results[1]
    return_result['predicted_0']=conf_matrix_results[2]
    return_result['predicted_1']=conf_matrix_results[3]
    return_result['f1_score']=conf_matrix_results[4]
    
    return_result['datetime']=d
    json_result=json.dumps(return_result)
    
    url = 'http://localhost:8000/api/result'
    x = requests.post(url, json_result)
    print(x.text)
